import { createCandidate, getCandidateById } from '../../api/candidatesApi';
...
describe('createCandidate', () => {
  ...
  it('creates a candidate with CV using multipart FormData', async () => {
    fetchMock.mockResolvedValue(
      mockResponse(201, { success: true, data: { ...mockCandidate, cv: { id: 1, filename: 'cv.pdf', contentType: 'application/pdf', sizeBytes: 1024, uploadedAt: '2024-01-01T00:00:00.000Z' } } })
    );
  ...
  it('throws ApiError with fieldErrors on 400 response', async () => {
    ...
    await expect(
      createCandidate({ firstName: 'Jane', lastName: 'Doe', email: 'bad-email' })
...
describe('getCandidateById', () => {
  ...
  it('throws ApiError with status 404 when candidate not found', async () => {
    ...
    await expect(getCandidateById(999)).rejects.toThrow(ApiError);
